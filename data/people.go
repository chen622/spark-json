...
func NewPeople(id int) *People {
	...
	for i := 0; i <= rand.Intn(2); i++ { //随机产生2位或者3位的名
		first = fmt.Sprint(firstName[rand.Intn(firstNameLen)])
	}
	...
	return &People{Id: id, Name: fmt.Sprintf("%s%s", fmt.Sprint(lastName[rand.Intn(lastNameLen)]), first), Age: rand.Intn(100), Gender: gender, Friend: &People{Name: "friend"}}
}

func AddChild(father *People, i int) *People {
	father.Friend = NewPeople(i)
	return father.Friend
}